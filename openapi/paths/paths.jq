...
def paths_like(like; mark_p; del_p):
    debug("++ paths_like(\(like); \(mark_p); \(del_p))") |
    .paths 
    | with_entries(select(.key|test(like))) 
    | (if mark_p then .|mark_deprecated elif del_p then .|del_deprecated else . end)
    ;
